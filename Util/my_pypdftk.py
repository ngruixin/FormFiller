...
Adapted from https://github.com/revolunet/pypdftk
...
if os.getenv('PDFTK_PATH'):
    ...
    PDFTK_PATH = '/usr/bin/pdftk'       # TO SET ACCORDINGLY
...
def modify_xfdf(form, data={}):
    '''
    Gets all field names that are similar to the keys in data. 
    Uses the actual field name stored in "FieldName" as keys to 
    in @modified_data. 
    '''
    fields = []
    modified_data = {}
    for key, value in data.items():
        try: 
            keys = key.split(" ")
            cmd = "pdftk %s dump_data_fields | grep 'FieldName: ' | cut -d ':' -f 2 " % form
            for word in keys:
                cmd += "| grep -i %s " % word
            ...
            #print(output)
            for field in output:
                modified_data[field.strip()] = value
        except (Exception):
            pass
